...
export default function ShowAds() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('http://13.235.134.187:3000/admin/showAds', {
                    method: 'GET',
                    credentials: 'include',
                });
    ...
    const handleDeleteImage = async () => {
        try {
            const response = await fetch(`http://13.235.134.187:3000/admin/remove-ads/${selectedAdId}`, {
                method: 'DELETE',
                credentials: 'include',
    ...
    const handleDeleteClick = (id) => async () => {
        ...
        if (confirmDelete) {
            try {
                const response = await fetch(`http://13.235.134.187:3000/admin/deleteAd/${id}`, {
                    method: 'DELETE',
                    credentials: 'include',
                    headers: { 'Content-Type': 'application/json' },
                });
